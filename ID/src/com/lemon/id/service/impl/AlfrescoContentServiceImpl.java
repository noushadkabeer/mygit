...
public class AlfrescoContentServiceImpl implements AlfrescoContentService {
	...
	public String submitDetails(String username, String address, String details) throws ConnectionFailedException, IOException{
		// TODO Auto-generated method stub
			...
			String strURL = propS.getKeyValue("contentServerURL")
					+ propS.getKeyValue("serviceURL")+"submitDetail";
			String ticket = "";
			PostMethod post = new PostMethod(strURL);
			String uploadedPath = "";
			ticket = authService.readTicket();
			if (ticket != null && !ticket.isEmpty()	&& ticket.equals(authService.validateTicket(ticket))) {			
				post.addParameter("alf_ticket", ticket);
				try {
					System.out.println("Ticket generated.. writing details "+username+ strURL);
					Part[] parts = { new StringPart("fileName", username),
							new StringPart("address", address),
							new StringPart("details", details)};
					post.setRequestEntity(new MultipartRequestEntity(parts, post
							.getParams()));
					InputStream in = null;
					HttpClient client = new HttpClient();
